...
class PlotUtility():
    ...
    def __init__(self, show=True, suffix='', save=True, base_plot_dir=os.path.join('results', 'plots')):
        self.markersize = 8
        self.linewidth = 0.5
        self.show = show
        self.suffix = suffix
...
def plot_all_df_subplots(all_df, model_list, model_set_name, grouping_col, save, show,
    ...
    pl_util = PlotUtility(show=show, save=save, suffix='')
...
def plot_by_df(pl_util: PlotUtility, all_df, to_plot_models, model_set_name, grouping_col,
    ...
    for key, turn_df in grouped:
        ...
        for y_axis, x_axis in to_iter:
            ...
            df_groups = df_to_plot.groupby(['model_code'])[
                ['x', 'xerr', 'y', 'yerr', grouping_col]]
            n_lines = len(df_groups)
    ...
    for dataset_name in datasets:
        for base_model_code in ['lr', 'lgbm']:
            ...
            if not hybrids_results.empty:
                ...
                pl_util = PlotUtility(show=show, save=save, suffix='ONLY ORACLE CALLS' + suffix)
